...
def is_Emblem_sheet(ws):
    ...
    base_rng = findAllRange("base",ws,search_rng=search_area)
...
def create_emblem_columnH01(ws):
    ...
    rng_base = findAllRange("Base",curr_ws,search_rng=search_base)
    first_col_var = nextTextCell(rng_base,"down")
    ...
    while isinstance(first_col_var,xw.main.Range):
        ...
        level_rng = pickTilEnd(level_start,"down")
        variable_name = second_col_var.value
        level_list = level_rng.value

        # find columns untils the end
        first_col_var = nextTextCell(first_col_var,"right",cut_off=10)
